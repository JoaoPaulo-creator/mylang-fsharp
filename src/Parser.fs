...
type Parser(tokens: Token list) =
    ...
    let isAtEnd () =
        current >= tokens.Length || tokens[current].Type = MY_EOF
    ...
    let check tokenType =
        not (isAtEnd ()) && peek().Type = tokenType
    ...
    let consume tokenType message =
        if check tokenType then
            ...
            raise (RuntimeError(peek (), message))
    ...
    and ifStatment ()  =
        ignore (consume LEFT_PAREN "Expected '(' after 'if'.")
        let cond = expression ()
        ignore (consume RIGHT_PAREN "Expected ')' after condition.")
    ...
    and statement () =
        ...
        elif matchTokens [ IF ] then ifStatment ()
        elif matchTokens [ LEFT_BRACE ] then Block(block ())
        else expressionStmt ()

    and declaration () =
        try
            if matchTokens [ VAR ] then
                varDeclaration ()
            else
                statement ()
        with _ ->
